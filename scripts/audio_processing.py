...
def process_audio_source(audio_path, output_dir, min_silence_len=1000, silence_thresh=-40, keep_silence=200):
    standardized_audio_path = os.path.join(output_dir, f"standardized_{os.path.basename(audio_path)}.wav")
    standardized_audio_path = process_audio_file_standardize(audio_path, standardized_audio_path)
    ...
    try:
        ...
        for i, segment in enumerate(segments):
            segment_output_path = os.path.join(output_dir, f"{os.path.splitext(os.path.basename(audio_path))[0]}_segment_{i}.wav")
        ...
        logging.info(f"Successfully processed and segmented audio from {audio_path}. Found {len(segments)} segments.")
