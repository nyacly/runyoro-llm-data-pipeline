...
def process_video_source(video_path, audio_output_dir, segment_params=None):
    ...
    extracted_audio_path = os.path.join(
        audio_output_dir, f"extracted_audio_{os.path.basename(video_path)}.wav"
    )
    extracted_audio_path = extract_audio_from_video_ffmpeg(
        video_path, extracted_audio_path
    )
    ...
    audio_proc_result = process_audio_source(
        extracted_audio_path, audio_output_dir, **(segment_params or {})
    )
    return audio_proc_result.get("segments", [])
