...
def process_data_source(
    ...
    if os.path.exists(metadata_filepath):
        try:
            ...
            logging.warning(
                f"Metadata file {metadata_filepath} is corrupted or empty. Starting fresh."
            )
            existing_metadata = []
    ...
    for entry in existing_metadata:
        if entry.get("original_source") == original_source_identifier and (
            original_source_content_hash is None
            or entry.get("original_source_hash") == original_source_content_hash
        ):
            logging.info(
                f"Skipping duplicate source: {source_path_or_url} (already processed)."
            )
            is_duplicate = True
            break
    ...
    if source_type in [
        ...
        processed_filepath = process_text_source(
            source_path_or_url, source_type, text_output_dir
        )
        ...
        audio_result = process_audio_source(
            source_path_or_url, audio_output_dir, **(segment_params or {})
        )
        for segment_info in audio_result.get("segments", []):
            processed_outputs_metadata.append(
                {
                    "source_type": source_type,
                    "original_source": original_source_identifier,
        ...
        os.makedirs(audio_output_dir, exist_ok=True)
        processed_segments = process_video_source(
            source_path_or_url, audio_output_dir, segment_params
        )
            ...
            processed_outputs_metadata.append(
                {
                    ...
                    "processed_audio_paths": [
                        seg["path"] for seg in pair["processed_audio"]
                    ],
